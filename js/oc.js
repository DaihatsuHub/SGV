...
function renderOC(){
  ...
  if(!window._ocLoaded){
    if(!window._ocLoading){
      ...
      (async()=>{
        try{
          if(typeof sbLoadOC==='function')      await sbLoadOC();
          if(typeof sbLoadOCItems==='function') await sbLoadOCItems();
          if(typeof sbLoadOCPagos==='function') await sbLoadOCPagos();
          if(typeof ensureArts==='function')    await ensureArts();
        }catch(e){ console.error('carga OC:',e); }
        window._ocLoaded=true; window._ocLoading=false; renderOC();
      })();
    }
    return;
  }
