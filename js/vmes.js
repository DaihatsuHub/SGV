...
function vmesRenderMarcas(){
  ...
  box.innerHTML = marc.length
    ? marc.map(m=>`<label style="display:flex;align-items:center;gap:4px;font-size:12px;color:var(--txt);cursor:pointer">
        <input type="checkbox" value="${esc(m.CODIGO)}" style="accent-color:var(--acc);cursor:pointer"> ${esc(m.CODIGO)}${m.DETALLE?' — '+esc(m.DETALLE):''}</label>`).join('')
...
async function vmesGenerar(){
  ...
  (ARTS||[]).forEach(a=>{
    ...
    rows.push({
      cod:art, pr:(a.ART_PROV||'').trim(), codcasio:(a.CODCASIO||'').trim(),
      mes, stock,
...
function vmesRender(rows, months){
  ...
  head.innerHTML=
    `<span>Código</span><span>PR</span><span>Cód.Casio</span>`
  ...
  body.innerHTML=rows.map(r=>{
    return `<div class="tr-art" style="grid-template-columns:${grid}">
      ...
      <span class="col-sm" style="font-family:var(--mono)">${esc(r.codcasio)}</span>
