...
function clrArtForm(){
  ['af-cod','af-des','af-grup','af-sex','af-tapa','af-codcasio'].forEach(i=>{ const el=document.getElementById(i); if(el) el.value=''; });
  ['af-pre','af-stk','af-stkt'].forEach(i=>{ const el=document.getElementById(i); if(el) el.value=0; });
  ['af-act','af-estu'].forEach(i=>{ const el=document.getElementById(i); if(el) el.value='S'; });
  fillArtSelects('','','','');
}
...
function fillArtForm(a){
  ...
  document.getElementById('af-act').value    = a.ART_ACT||'S';
